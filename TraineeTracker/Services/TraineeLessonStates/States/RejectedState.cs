...
namespace TraineeTracker.Services.TraineeLessonStates.States {
    ...
    /// <summary>
    /// Represents the 'Rejected' state of a trainee lesson and defines the allowed transitions from this state.
    /// </summary>
    /// <remarks>
    /// Code Ownership: Simon Hinterreiter (hintsimo)
    /// </remarks>
    public class RejectedState : ITraineeLessonState {

        // ----------------------------------------------
        /// <summary>
        /// Returns a list of lesson states that the current user is allowed to transition to from the 'Rejected' state.
        /// </summary>
        ...
        /// from the current 'Rejected' state based on the user's role.
        /// </returns>
        /// <remarks>
        /// Code Ownership: Simon Hinterreiter (hintsimo)
        /// </remarks>
        ...
        // ----------------------------------------------
        /// <summary>
        /// Attempts to transition from the 'Rejected' state to the specified target state, 
        ...
        public TraineeLessonState TransitionTo(TraineeLessonState targetState, ClaimsPrincipal user) {
            ...
            if (!allowed.Contains(targetState)) {
                throw new InvalidOperationException($"Transition from 'Rejected' to '{targetState}' is not allowed for this user.");
            }
            return targetState;
        }
